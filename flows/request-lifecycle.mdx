...
| `ProgressiveLoanModelCheckerFilter` | Ensures a progressive loan model entry exists for write endpoints. See [Progressive Loan Architecture](/progressive-loan/overview). |
...
The platform's global exception translation lives in `org.apache.fineract.infrastructure.core.exception.ErrorHandler` and Jersey `ExceptionMapper`s registered in `JerseyConfig`. See [Error Handling](/validation/overview) for the mapping table.
