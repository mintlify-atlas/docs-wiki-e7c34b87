...
The `LoanCOB` Spring Batch job runs nightly per [Loan COB](/cob/loan-cob-business-steps), and one of its registered business steps is `LoanAccountOwnerTransferBusinessStep`. The step is gated by `@Conditional(InvestorModuleIsEnabledCondition.class)` — it only runs when the investor module is active.
