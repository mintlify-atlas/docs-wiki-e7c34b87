...
- [Content store](/document/content-store) — `ContentStoreService` SPI that runs these policies on every method.
- [S3 and filesystem storage](/document/s3-and-filesystem-storage) — concrete backends that invoke the chain.
- [Document API](/document/document-api) — caller for documents, with `ContentDetectorManager` injected for MIME fallback.
- [Images API](/document/images-api) — caller for avatars, with the data-URL and base64 processors composed alongside.
