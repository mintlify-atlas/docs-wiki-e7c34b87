...
- [Domain model](document-management-domain) — `Document` entity, `DocumentRepository`, request DTOs.
- [Images API](images-api) — sibling resource for avatar images.
- [Content store](content-store) — `ContentStoreService` SPI.
- [Detector and policies](detector-and-policies) — Tika and the policy chain that gate every upload.
