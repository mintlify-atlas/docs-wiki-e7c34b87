...
description: "Component graph, request lifecycle, runtime layers and per-layer file map for the Apache Fineract Spring Boot monolith and its 19 Gradle modules."
---
...
| File | Role |
|------|------|
| `fineract-core/src/main/java/org/apache/fineract/batch/api/BatchApiResource.java` | HTTP-level batch endpoint that enforces `fineract.mode.read-enabled` via `fineractProperties.getMode().isReadOnlyMode()`. |
...
| File | Role |
|------|------|
| `fineract-core/src/main/java/org/apache/fineract/infrastructure/core/config/HikariCpConfig.java` | HikariCP defaults shared across tenants. |
| `fineract-core/src/main/java/org/apache/fineract/infrastructure/core/config/JdbcConfig.java` | Bootstraps the **tenant-master** DataSource. |
| `fineract-core/src/main/java/org/apache/fineract/infrastructure/core/service/database/DataSourcePerTenantServiceFactory.java` | Builds per-tenant Hikari pools; respects `fineract.tenant.read-only-host` and `isReadOnlyMode()`. |
