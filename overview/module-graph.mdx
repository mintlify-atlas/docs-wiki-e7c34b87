...
| 13 | `:fineract-tax` | Fineract Taxes | Tax components, groups, mappings. |
...
| 20 | `:integration-tests` | Fineract Integration Tests | Black-box REST tests. |
| 21 | `:twofactor-tests` | Fineract Integration Tests | Two-factor authentication test pack. |
| 22 | `:oauth2-tests` | Fineract Integration Tests for OAuth2 | OAuth2 authentication test pack. |
| 23 | `:fineract-client` | Fineract Client | Retrofit-based REST client. |
| 24 | `:fineract-client-feign` | Fineract Client with Feign | Spring Cloud OpenFeign variant. |
| 25 | `:fineract-doc` | _(no `description`)_ | Antora/AsciiDoc developer documentation. |
| 26 | `:fineract-avro-schemas` | Fineract Avro Schemas | Avro IDL for external events. |
| 27 | `:fineract-e2e-tests-core` | _(no `description`)_ | Cucumber/Karate shared steps. |
| 28 | `:fineract-e2e-tests-runner` | _(no `description`)_ | Runnable harness. |
| 29 | `fineract-progressive-loan` | Fineract Progressive Loan | Newer progressive-interest loan engine. |
| 30 | `fineract-progressive-loan-embeddable-schedule-generator` | Fineract Progressive Loan Embeddable Schedule Generator | Embeddable schedule library. |
| 31 | `fineract-working-capital-loan` | Fineract Working Capital Loan | WCL product variant on top of `:fineract-loan`. |
| 32 | `:custom:docker` | _(custom image)_ | Aggregates every `custom/` module into a single Docker image. |
...
| `infrastructure/core/config/` | `FineractProperties` (`@ConfigurationProperties(prefix = "fineract")`), `AbstractFineractModuleProperties`, `ExplicitConfigurationPropertiesFactory`, `MapstructMapperConfig`. (`HikariCpConfig` and `JdbcConfig` are in `fineract-provider` and `@Import`ed into both web and `liquibase-only` boot configs.) |
