...
<Note>
The `Document` entity lives in **`fineract-document`** (not `fineract-core`) because document storage is an optional module — a deployment that doesn't need attachments can run without `fineract-document` on the classpath. Only `EntityImageIdAdapter` ships in `fineract-core`. See [the documents overview](/document/overview) for the full storage runtime.
</Note>
...
## Storage adapters (/core/overview)
...
See [the documents overview](/document/overview) for a full breakdown of storage adapters and configuration.
