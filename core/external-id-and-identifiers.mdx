...
public abstract class IdTypeResolver {
    ...
    public enum IdType implements DefaultOption {
        ...
        SHORT_NAME;     // /clients/short-name/…

        @Override
        public boolean isDefault() { return this == ID; }
    }

    public static IdType resolveDefault()             { return IdType.ID; }
    public static IdType resolve(String idType)       { /* parses to enum, throws PlatformApiDataValidationException on unknown */ }
}
```

`IdTypeResolver` itself is abstract; concrete resource subclasses extend it
to bind the resolved `IdType` to the entity-specific repository call. The
enum has three values rather than the binary internal/external pair you
might expect — a small number of resources (offices, codes) also accept the
`short-name` path style.
