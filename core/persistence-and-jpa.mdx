...
Implementations live across `fineract-provider` and the batch modules. To
register a callback, declare a `@Component` that implements the interface and
call `ExtendedJpaTransactionManager.addLifecycleCallback(myCallback)` from
your config — typically from a `@PostConstruct` on a configuration class.
