...
A deliberately empty marker class. The journal in Fineract is a **read-only roll-up** produced by `TellerManagementReadPlatformService.getJournals` and `fetchTellerJournals`, both of which return `Collection<TellerJournalData>` — see [Teller Journal API](teller-journal-api).
...
- [Branch overview](overview) — module map.
- [Teller API](teller-api) — REST surface that consumes these entities.
- [Cashier API](cashier-api) — allocate / settle endpoints.
- [Organisation / Offices](/organisation/offices) — the `Office` entity that owns a teller.
- [API / Teller APIs](/api/teller-apis) — the published REST reference.
