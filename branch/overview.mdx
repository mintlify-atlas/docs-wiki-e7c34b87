...
- **[Teller and Cashier Domain](/branch/teller-and-cashier-domain)** — the `Teller`, `Cashier`, `CashierTransaction`, and `TellerStatus` entities in detail.
- **[Teller API](/branch/teller-api)** — `TellerApiResource` endpoints with request and response shapes.
- **[Cashier API](/branch/cashier-api)** — POST cashier, allocate cash, settle cash flows.
- **[Teller Journal API](/branch/teller-journal-api)** — `TellerJournalApiResource` and journal queries.
- See also: [Organisation / Offices](/organisation/offices), [Portfolio / Clients](/portfolio/clients), [API / Teller APIs](/api/tellers).
