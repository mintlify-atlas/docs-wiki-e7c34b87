...
- The **REST surface** for defining charges → [Charges API](/charges-api).
...
Write permissions come from the `m_permission` rows seeded by Liquibase: `CREATE_CHARGE`, `UPDATE_CHARGE`, `DELETE_CHARGE`, plus the `*_CHECKER` variants when maker-checker is enabled. See the configuration / code overview at [Configuration and code APIs](/api/configuration-and-code-apis).
