...
The handler beans live in `fineract-provider/.../portfolio/loanaccount/handler/`. Each one is annotated `@CommandType(entity = "LOAN", action = "...")` and is found by the `CommandHandlerProvider` at command execution time. See [Command source pipeline](/command/command-source).
...
Both throw `PlatformApiDataValidationException` (mapped to HTTP 400) on failure — see [Error handling](/core/exception-mappers).
...
Every public method is `@Transactional`. The optimistic-lock `@Version` on `Loan` will throw `ObjectOptimisticLockingFailureException` if two operations race on the same loan; the runtime maps this to HTTP 409. See [Error handling](/core/exception-mappers) for the conversion details.
