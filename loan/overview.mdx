...
<CardGroup cols={2}>
  ...
  <Card title="Transaction processors" icon="filter" href="/loan/transaction-processors">
    The seven shipped allocation strategies — `mifos-standard`, `rbi-india`, `creocore`, `heavensfamily`, `early-repayment`, due-pen-int-pri-fee, and the progressive `advanced-payment-allocation-strategy`.
  </Card>
...
- **[Loan APIs (catalog)](/api/loan-apis)** — Swagger-style catalog of every endpoint with request/response shapes if you only need the wire contract.

## Module boundaries and dependencies

`fineract-loan` depends on `fineract-core` (for `MonetaryCurrency`, `Money`, `JsonCommand`, `CommandWrapper`, `ExternalId`, `AbstractAuditableWithUTCDateTimeCustom`, `LoanStatus`) and on `fineract-loan-product` (for `LoanProduct`, `LoanProductRelatedDetail`, `InterestMethod`, `AmortizationType`). It is consumed by:
