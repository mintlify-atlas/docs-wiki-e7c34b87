...
The module ships its command handlers in `service/`, not `handler/`. The `service/*Handler.java` files implement `NewCommandSourceHandler` and are wired into the command-source pipeline via the `@CommandType` annotation — see [Portfolio command source](/command/command-source).
...
- Investor REST API reference: [/api/investor-apis](/api/external-asset-owners)
