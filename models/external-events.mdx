...
| `ExternalEventView` (Spring Data projection over `m_external_event`) | `event.external.repository.domain.ExternalEventView`                | projection interface — no separate table; consumed by `ExternalEventRepository.findByStatusOrderByBusinessDateAscIdAsc` |
...
## `ExternalEventView` (Spring Data projection)

`ExternalEventView` (in
`fineract-core/src/main/java/org/apache/fineract/infrastructure/event/external/repository/domain/ExternalEventView.java`)
is **not** a database view — it is a Spring Data
[interface projection](https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#projections.interfaces)
declared as a Java `interface` over `ExternalEvent`. Spring Data JPA builds a
proxy that exposes a stable read-only contract (`getId`, `getType`,
`getCategory`, `getSchema`, `getData`, `getCreatedAt`, `getStatus`,
`getSentAt`, `getIdempotencyKey`, `getBusinessDate`, `getAggregateRootId`)
and lets the underlying entity evolve without affecting the producer code
path. It is consumed by
`ExternalEventRepository.findByStatusOrderByBusinessDateAscIdAsc`, which
returns `List<ExternalEventView>` for the
`SendAsynchronousEventsTasklet` polling loop.
...
`SendAsynchronousEventsTasklet` (in
`fineract-core/src/main/java/org/apache/fineract/infrastructure/event/external/jobs/SendAsynchronousEventsTasklet.java`)
runs on a Spring Batch schedule and polls `m_external_event` via the
projection. The effective query pattern is:

```sql
SELECT id, type, category, schema, data, created_at, status, sent_at,
       idempotency_key, business_date, aggregate_root_id
FROM   m_external_event
WHERE  status = 'TO_BE_SENT'
ORDER  BY business_date ASC, id ASC
LIMIT  ?  -- fineract.events.external.producer.read-batch-size
```

The repository call is
`findByStatusOrderByBusinessDateAscIdAsc(ExternalEventStatus.TO_BE_SENT,
PageRequest.of(0, batchSize))` and returns
`List<ExternalEventView>`. Each batch is shipped to the configured broker
(JMS or Kafka, selected by `KafkaExternalEventProducer` /
`JmsExternalEventProducer`), then UPDATEd in a second transaction to
`status = 'SENT'` with `sent_at = now()`. Failed ships leave the rows in
`TO_BE_SENT` and the next tasklet tick picks them up.
