...
| (no separate `m_pentaho_reports` table) | Pentaho reports live as rows in `stretchy_report` with `report_type = 'Pentaho'`; the `.prpt` files are loaded from `${FINERACT_BASE_DIR}/pentahoReports` by `ReadReportingServiceImpl` | seeded by `0018_pentaho_reports_to_table.xml` (inserts into `stretchy_report`) |
...
`0018_pentaho_reports_to_table.xml` does **not** add any Pentaho-specific
columns — it only `<insert>`s rows into `stretchy_report` so the canonical
Pentaho-driven reports (Trial Balance, General Ledger, etc.) ship with the
schema. The Pentaho engine itself reads the `.prpt` files from
`${FINERACT_BASE_DIR}/pentahoReports` at runtime; see
`ReadReportingServiceImpl.processPentahoRequest`.
