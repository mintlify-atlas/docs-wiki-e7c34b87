...
- For the broader report API surface and the relationship with `/v1/runreports/{name}`: [Reports and data APIs](/api/reports).
- For the chart of accounts the JS expressions read against: accounting module reference and [Configuration and code APIs](/api/global-configuration).
