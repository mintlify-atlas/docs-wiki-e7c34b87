...
The REST resource at `/v1/hooks` is a thin wrapper around the command framework — all writes go through `PortfolioCommandSourceWritePlatformService` so they participate in maker-checker and audit logging. See [api/hooks-and-messaging-apis](/api/hooks) for the full request/response shapes.
...
If you need at-least-once delivery, durable retries, or a dead-letter topic, prefer the [external event](/events/external-event-domain) bus (Kafka/JMS), not hooks.
...
- API: [Hooks & messaging APIs](/api/hooks).
