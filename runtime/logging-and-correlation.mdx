...
| `<if condition="${jsonLoggingEnabled} == true">` | Switches between text console output and `JsonLayout`. The `<if>` element requires the Janino bundle (provided transitively by Logback). |
